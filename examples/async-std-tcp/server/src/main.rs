...
fn main() {
    ...
    let authenticator_mode =
        AuthenticatorMode::RequireTcp(lyanne::auth_tcp::AuthTcpServerProperties {
...
fn use_tick_result(server: &Server, tick_result: ServerTickResult) {
    for (auth_entry, message) in tick_result.to_auth {
            ...
            .downcast::<HelloPacket>()
        {
            ...
            server.authenticate(auth_entry, server.packet_registry().empty_serialized_list());
        } else {
            println!(
