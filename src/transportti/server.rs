        ...
        if now - connected_client.last_response >= TIMEOUT {
                ...
                connected_client.pending_packets_confirmation.get(0)
            {
                if let Some(packets_to_process) = responses.remove(serialized_cache_index) {
                    ...
                    if sent_count >= MAX_PENDING_MESSAGES_STACK {
                        break;
                    }
                    println!(
            ...
            if now - connected_client.last_response >= TIMEOUT {
                ...
                to_disconnect.push(*addr);
                continue;
            } else if sent_count < MAX_PENDING_MESSAGES_STACK {
                connected_client.last_response = Instant::now();

                println!(
                    "  sending packet {:?} to client, and caching it to handle packet loss",
                    cache_index
                );
                send(&server_read, &runtime, &addr, cache_index, &packets);
                connected_client
